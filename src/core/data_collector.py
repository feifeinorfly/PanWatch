...
class DataCollectorManager:
    ...
    async def collect_kline(
        ...
        """采集 K 线数据(优先走 KlineOrchestrator,失败回退内置采集器)"""
        from src.collectors.kline_collector import KlineCollector
        from src.models.market import MarketCode
        ...
        try:
            ...
            try:
                ...
                resp = await get_kline_orchestrator().fetch(req, cache_ttl_sec=60)
                if resp.success and resp.data:
                    collector = KlineCollector(market_code)
                    summary = collector.get_kline_summary(klines=resp.data)
            except Exception:
                pass
    ...
    async def _test_source_impl(
        ...
        if source.type == "news":
            ...
            if collector:
                if source.provider == "xueqiu":
                    try:
                        news = await asyncio.wait_for(
                            collector.fetch_news(symbols=test_symbols, since=since),
                            timeout=60,
                        )
                        ...
                        return CollectorResult(
                            ...
                            data=[],
                            count=0,
                            error="雪球资讯测试超时：Playwright 初始化超过 60 秒，请检查 Docker 资源或网络",
                        )
                else:
                ...
                if len(news) == 0:
                    if source.provider == "xueqiu":
                        # 优先使用采集器记录的 last_error（更精确）
                        collector_error = getattr(collector, "last_error", "")
                        if not cookies:
                            error_msg = "雪球 Cookie 为空，请在数据源配置中填写有效的雪球登录 Cookie"
                        elif collector_error:
                            error_msg = collector_error
                        else:
    ...
    async def _test_kline_source(
        ...
        elif source.provider == "tdx":
            provider = TdxKlineProvider(config=cfg)
            market = "CN"
            if provider._init_error:
                return CollectorResult(
                    success=False, error=provider._init_error
